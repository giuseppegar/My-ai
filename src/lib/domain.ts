...
export type Memory = { id: string; title: string; category: Category; kind: MemoryKind; content: string; origin: string; created_at: string; updated_at: string; auto_conversation_id?: string | null };
export type Conversation = { id: string; title: string; category: Category; created_at: string; searchable: boolean };
...
export type Document = { id: string; title: string; category: Category; filename: string; mime: string; scope: 'chat' | 'memory'; conversation_id: string | null; path: string; status: 'processing' | 'ready' | 'no_text' | 'error'; error: string | null; page_count: number | null; origin: 'upload' | 'generated'; created_at: string; preview_url?: string; size_bytes?: number };
...
export type Bootstrap = { capabilities: Capabilities; user: { id: string; email: string } | null; memories: Memory[]; conversations: Conversation[]; documents: Document[]; wishes: Wish[]; sessions: AgentSession[]; videoJobs: VideoJob[]; preferences: Preferences; gauges: Gauge[] };
