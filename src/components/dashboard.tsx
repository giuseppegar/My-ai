...
const titles: Record<View, string> = { home: 'Il tuo spazio di pensiero', collection: 'L’Arcipelago della Memoria', history: 'I fili delle tue conversazioni', documents: 'I tuoi documenti, al loro posto', wishes: 'Un passo verso ciò che desideri', settings: 'A modo tuo' };
export function Dashboard() {
  ...
  return <div className={data.preferences.reduced_motion ? 'app reduce-motion' : 'app'}>
    ...
    <main id="main" className="main-container">
      <div className="page-eyebrow"><span><span className="little-line"/>{view === 'home' ? 'UNO SPAZIO PER COLTIVARE LE TUE IDEE' : view === 'collection' ? 'LE TUE ISOLE E LA MEMORIA CATALOGATA' : 'IL TUO SPAZIO PERSONALE'}</span><span className="today">{new Intl.DateTimeFormat('it-IT', { weekday: 'long', day: 'numeric', month: 'long' }).format(new Date())}</span></div>
      ...
      {!loaded ? <div className="loading-home" role="status"><LoaderCircle className="spin" size={25}/>Preparo il tuo spazio…</div> : !canUse && view !== 'settings' ? <section className="welcome"><Island category="desideri"/><h1>Le tue idee meritano<br/>un posto a cui tornare.</h1><p>Una chat, quattro isole e una memoria che scegli tu.<br/>Accedi per ritrovare il tuo mondo, da qualsiasi dispositivo.</p><button className="button primary" onClick={() => setAuthOpen(true)}>Entra nel tuo arcipelago <ArrowRight size={17}/></button></section> : <>
        {view === 'home' && <>
          ...
          <section className="gauges-section" aria-labelledby="gauges-title">
            <div className="section-kicker">
              <div>
                <h2 id="gauges-title">Piccoli segni delle tue esplorazioni</h2>
                <span className="small muted">Attività nell’app, non punteggi personali</span>
              </div>
              <button className="button ghost small" aria-label="Come sono calcolati gli indicatori" onClick={() => setGauge(data.gauges[0])}>
                <CircleHelp size={14}/>
                <span>Dettagli indicatori</span>
              </button>
            </div>
            <div className="gauges-grid">
              {data.gauges.map((g, i) => <AnalogGauge key={g.label} gauge={g} index={i} onClick={() => setGauge(g)}/>)}
            </div>
          </section>
        </>}
        {view !== 'home' && view !== 'collection' && <div className="view-heading"><button className="button ghost small" onClick={() => navigate('home')}><ArrowLeft size={15}/>Il mio spazio</button><h1>{titles[view]}</h1></div>}
        {view === 'collection' && <>
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '12px' }}>
            <button className="button ghost small" onClick={() => navigate('home')}><ArrowLeft size={15}/>Torna alla Home</button>
            {selectedIsland && <span className="badge" style={{ borderColor: selectedIsland.color, color: selectedIsland.color }}>Isola selezionata: {selectedIsland.name}</span>}
          </div>
          <Archipelago data={data} category={category} selectedIslandId={selectedIslandId} disabled={chatBusy} onChat={(next, islId) => { chooseChat(next, islId); navigate('home'); }} onOpen={(next, islId) => { setCategory(next); setSelectedIslandId(islId || null); document.getElementById('cataloged-memories')?.scrollIntoView({ behavior: 'smooth' }); }} onList={() => { document.getElementById('cataloged-memories')?.scrollIntoView({ behavior: 'smooth' }); }} onConsolidate={handleConsolidate} isConsolidating={isConsolidating}/>
          <div id="cataloged-memories" className="cataloged-section" style={{ marginTop: '38px', scrollMarginTop: '100px' }}>
            <div className="section-heading" style={{ marginBottom: '16px' }}>
              <div>
                <p className="eyebrow">ARCHIVIO VIVO</p>
                <h2 style={{ fontSize: '26px' }}>Tutta la memoria catalogata</h2>
                <p>{selectedIsland ? `Ricordi e documenti custoditi nell’isola “${selectedIsland.name}”.` : selectedCategory ? `Ricordi e documenti dell’ambito “${selectedCategory.name}”.` : 'Tutti i ricordi, idee e scoperte organizzati e custoditi nelle tue isole.'}</p>
              </div>
              {(selectedIslandId || category) && (
                <button className="button ghost small" onClick={() => { setSelectedIslandId(null); setCategory(null); }}><X size={14}/>Mostra tutte le isole</button>
              )}
            </div>
            <div className="toolbar"><div className="filter-search"><Search size={17}/><input aria-label="Cerca nella memoria catalogata" value={filter} placeholder="Cerca nella memoria catalogata…" onChange={e => setFilter(e.target.value)}/></div><select aria-label="Filtra per isola" value={selectedIslandId || category || ''} onChange={e => { const val = e.target.value; const isIsland = data.islands?.some(i => i.id === val); if (isIsland) { setSelectedIslandId(val); setCategory(null); } else { setSelectedIslandId(null); setCategory(val as Category || null); } }}><option value="">Tutte le isole</option>{data.islands && data.islands.length > 0 && <optgroup label="Le tue Isole">{data.islands.map(i => <option key={i.id} value={i.id}>{i.name}</option>)}</optgroup>}<optgroup label="Ambiti base">{categories.map(c => <option key={c.id} value={c.id}>{c.name}</option>)}</optgroup></select><select aria-label="Filtra ricordi per tipo" value={kindFilter} onChange={e => setKindFilter(e.target.value)}><option value="">Tutti i tipi</option>{Object.entries(kindLabels).map(([key,value]) => <option value={key} key={key}>{value}</option>)}</select><button className="button primary" onClick={() => setMemoryEditor({ category: category || 'capire', island_id: selectedIslandId || null })}><Plus size={16}/>Nuovo ricordo</button></div>
            {!filteredMemories.length ? <Empty title={filter || selectedIslandId || kindFilter ? "Nessun ricordo trovato" : "Qui c’è spazio per qualcosa di bello"}>{filter || selectedIslandId || kindFilter ? "Non ci sono ricordi che corrispondono ai filtri selezionati. Prova a reimpostare i filtri." : "Le nuove richieste vengono salvate qui automaticamente, tranne quelle nelle chat riservate. Puoi anche aggiungere un ricordo con le tue parole."}</Empty> : <div className="memory-grid">{filteredMemories.map(m => { const Icon = icons[m.category]; const mIsland = data.islands?.find(i => i.id === m.island_id); return <article className={`memory-card category-${m.category}`} key={m.id}><div className="card-meta"><span><Icon size={15}/>{mIsland ? mIsland.name : categoryOf(m.category).name}</span><span>{kindLabels[m.kind]}</span></div><button className="card-main" onClick={() => { void perform(() => openContent({ id: m.id, source: 'memory' })); }}><h3>{m.title}</h3><p>{m.content.replace('ESEMPIO DIMOSTRATIVO', '').slice(0, 200)}</p></button><div className="card-footer"><span>{dateLabel(m.created_at)}</span><button className="icon-button" aria-label={`Modifica ${m.title}`} onClick={() => setMemoryEditor(m)}><Pencil size={15}/></button><button className="icon-button" aria-label={`Elimina ${m.title}`} onClick={() => { void perform(() => remove('memories', m.id, m.title)); }}><Trash2 size={15}/></button></div></article>; })}</div>}
            {data.documents.some(d => d.scope === 'memory' && (!category || d.category === category) && (!selectedIslandId || d.island_id === selectedIslandId)) && <section className="panel collection-docs"><h3>Documenti in quest’isola</h3>{data.documents.filter(d => d.scope === 'memory' && (!category || d.category === category) && (!selectedIslandId || d.island_id === selectedIslandId) && d.title.toLowerCase().includes(filter.toLowerCase())).map(d => <button className="list-row" key={d.id} onClick={() => { void perform(() => openContent({ id: d.id, source: 'document' })); }}><FileText size={18}/><span>{d.title}</span><ArrowRight size={16}/></button>)}</section>}
            {category === 'desideri' && <button className="button secondary" style={{ marginTop: '16px' }} onClick={() => navigate('wishes')}><Flag size={16}/>Apri i percorsi dei desideri</button>}
          </div>
        </>}
