...
export function Dashboard() {
  ...
  const ensureConversation = async (text: string) => {
    ...
    const created = await api<Conversation>('conversations', 'POST', { title: text.trim().slice(0, 100) || 'Una nuova conversazione', category: category || proposeCategory(text) });
  ...
  const chooseChat = (next: Category | null) => {
    if (chatBusy) return;
    if (next !== category || (conversation && next !== conversation.category)) { setConversation(null); setMessages([]); setRefs([]); setUploadResults([]); }
    setCategory(next); focusChat();
  };
  ...
  const submit = async (text: string, files: PendingFile[], mode: { deep: boolean; web: boolean; image: boolean; video: boolean; artifact: boolean }, uploaded: (id: string) => void, onlyFiles: boolean) => {
    ...
    if (mode.image || mode.video || mode.artifact) {
      ...
      try {
        const input = { prompt: text, conversation_id: conv.id, category: category || proposeCategory(text) };
        if (mode.video) {
    ...
    const input = { content: text, conversation_id: conv.id, category: category || proposeCategory(text), refs: refs.map(({ id, source }) => ({ id, source })), web: mode.web };
    try {
  ...
  const filteredMemories = data.memories.filter(m => (!category || m.category === category) && (!kindFilter || m.kind === kindFilter) && `${m.title} ${m.content}`.toLowerCase().includes(filter.toLowerCase()));
  ...
  return <div className={data.preferences.reduced_motion ? 'app reduce-motion' : 'app'}>
    ...
    <main id="main" className="main-container">
      ...
      {!loaded ? <div className="loading-home" role="status"><LoaderCircle className="spin" size={25}/>Preparo il tuo spazio…</div> : !canUse && view !== 'settings' ? <section className="welcome"><Island category="desideri"/><h1>Le tue idee meritano<br/>un posto a cui tornare.</h1><p>Una chat, quattro isole e una memoria che scegli tu.<br/>Accedi per ritrovare il tuo mondo, da qualsiasi dispositivo.</p><button className="button primary" onClick={() => setAuthOpen(true)}>Entra nel tuo arcipelago <ArrowRight size={17}/></button></section> : <>
        {view === 'home' && <>
          <Archipelago data={data} category={category} disabled={chatBusy} onChat={chooseChat} onOpen={next => { setCategory(next); navigate(next === 'desideri' ? 'wishes' : 'collection'); }} onList={() => { setCategory(null); navigate('collection'); }}/>
          <section id="chat-workspace" className={`chat-section chat-island category-${category || 'libera'}`} aria-labelledby="chat-title"><div className="chat-island-heading"><span className="badge">{selectedCategory?.name || 'Chat libera'}</span><button className="button ghost small" onClick={() => document.getElementById('islands-title')?.scrollIntoView({ block: 'start' })}>Torna alle isole ↑</button></div><div className="chat-intro"><div className="chat-spark"><Sparkles size={19} strokeWidth={1.3}/></div><h2 id="chat-title">{selectedCategory ? `Parliamo di ${selectedCategory.name.toLowerCase()}` : data.preferences.display_name ? `Cosa esploriamo, ${data.preferences.display_name}?` : 'La tua isola per parlare liberamente'}</h2><p>Ritrova un ricordo. Fai spazio a un’idea. Segui una curiosità.</p></div>
            ...
            {selectedCategory && <div className="context-category"><span className={`badge category-${selectedCategory.id}`}>Contesto: {selectedCategory.name}</span><button className="button ghost small" disabled={chatBusy} onClick={() => chooseChat(null)}>Scrivi liberamente <X size={13}/></button></div>}
        ...
        {view === 'collection' && <>
          <div className="toolbar"><div className="filter-search"><Search size={17}/><input aria-label="Cerca nella raccolta" value={filter} placeholder="Cerca in questa raccolta…" onChange={e => setFilter(e.target.value)}/></div><select aria-label="Filtra per isola" value={category || ''} onChange={e => setCategory(e.target.value as Category || null)}><option value="">Tutte le isole</option>{categories.map(c => <option key={c.id} value={c.id}>{c.name}</option>)}</select><select aria-label="Filtra ricordi per tipo" value={kindFilter} onChange={e => setKindFilter(e.target.value)}><option value="">Tutti i ricordi</option>{Object.entries(kindLabels).map(([key,value]) => <option value={key} key={key}>{value}</option>)}</select><button className="button primary" onClick={() => setMemoryEditor({ category: category || 'capire' })}><Plus size={16}/>Nuovo ricordo</button></div>
